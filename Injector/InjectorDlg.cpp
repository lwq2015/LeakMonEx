...
BEGIN_MESSAGE_MAP(CInjectorDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_FIND, &CInjectorDlg::OnBnClickedFind)
END_MESSAGE_MAP()
...
BOOL CInjectorDlg::OnInitDialog()
{
	...
	m_List.InsertColumn(0, _T("PID"), LVCFMT_LEFT, 60);
	m_List.InsertColumn(1, _T("Process"), LVCFMT_LEFT, 160);
	m_List.InsertColumn(2, _T("Path"), LVCFMT_LEFT, 600);
...
void CInjectorDlg::Refresh()
{
	...
	do
	{
		...
		if (auto hProc = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE, stProcess.th32ProcessID))
		{
			auto nRet = GetModuleFileNameEx(hProc, nullptr, szImgPath, MAX_PATH);
			CloseHandle(hProc);

			if (nRet)
			{
				m_List.SetItemText(nPos, 2, szImgPath);
			}
		}
...
{
	const CInjectorDlg* pThis = (CInjectorDlg*)lParamSort;
	CString csText1 = pThis->m_List.GetItemText((int)lParam1, pThis->m_nColumnClick);
	CString csText2 = pThis->m_List.GetItemText((int)lParam2, pThis->m_nColumnClick);
	int nReturn = 0;
...
void CInjectorDlg::OnBnClickedFind()
{
	CString szFind, szItem;
	const int nRowCount = m_List.GetItemCount();

	GetDlgItemText(IDC_EDT_FIND, szFind);
	szFind.Trim();
	if (szFind.IsEmpty())
	{
		MessageBox(_T("请输入搜索关键字"), _T("提示..."), MB_ICONERROR | MB_OK);
		return;
	}
	...
	for (int i = iSel >= 0 ? iSel + 1 : 0; i < nRowCount; i++)
	{
		szItem = m_List.GetItemText(i, 1);
		szItem.MakeLower();

		if (szItem.Find(szFind) >= 0)
		{
			m_List.SetFocus();
			m_List.SetItemState(i, LVIS_SELECTED | LVIS_FOCUSED, LVIS_SELECTED | LVIS_FOCUSED);
			m_List.EnsureVisible(i, FALSE); // 滚动到可视区域
			return;
		}
	}

	MessageBox(_T("未搜索到"), _T("提示..."), MB_ICONINFORMATION | MB_OK);
}
