...
{ \
    s_pfnOrg##Function = (Function##Def)GetProcAddress(Dll, #Function); \
    if ( s_pfnOrg##Function != NULL )\
    {\
		HOOKFUNCDESC entry; \
		entry.pProc = (PROC)My##Function; \
		entry.pOrigProc = (PROC)s_pfnOrg##Function; \
		...
		OutputDebugStringA("#Leakmon: Failed to get address of function:" #Function "@" #Dll ".dll\n"); \
    }\
}
//////////////////////////////////////////////////////////////////////////////////
...
E_HOOK_TYPE g_HookType = HT_UNKNOWN;
int g_StackDepth = 20;
...
void StackDump(LPVOID pMem, DWORD dwBytes)
{
	...
	for (int i = 0; i < g_StackDepth; i++)// only retrieve 40 functions
	{
		...
		//////////////////////////////////////////////////////////////////////////
				...
				//{
				//		CString csLine = "Ordinal823";
				//	if( csLine == pSymbol->Name)
				//		{
				//			break;
				//		}
				//			
...
LPVOID WINAPI MyHeapAlloc(IN HANDLE hHeap, IN DWORD dwFlags, IN SIZE_T dwBytes)
{
	LPVOID lMem = s_pfnOrgHeapAlloc(hHeap, dwFlags, dwBytes);
	CreateCallStack(lMem, dwBytes);
	return lMem;
}

LPVOID WINAPI MyHeapReAlloc(HANDLE hHeap, DWORD dwFlags, LPVOID lpMem, SIZE_T dwBytes)
{
	LPVOID lpNewMem = s_pfnOrgHeapReAlloc(hHeap, dwFlags, lpMem, dwBytes);
	try
	{
		CSingleLock lockObj(&SyncObj, TRUE);
		if (g_bHooked && g_bTrack)
...
{
	LPVOID lMem = s_pfnOrgVirtualAlloc(lpAddress, dwSize, flAllocationType, flProtect);
...
BOOL WINAPI MyVirtualFree(LPVOID lpAddress, SIZE_T dwSize, DWORD dwFreeType)
{
	...
	return s_pfnOrgVirtualFree(lpAddress, dwSize, dwFreeType);
}

LPVOID WINAPI MyVirtualAllocEx(HANDLE hProcess, LPVOID lpAddress, SIZE_T dwSize, DWORD flAllocationType, DWORD flProtect)
{
	LPVOID lMem = s_pfnOrgVirtualAllocEx(hProcess, lpAddress, dwSize, flAllocationType, flProtect);
	CreateCallStack(lMem, dwSize);
	return lMem;
}
...
{
	RemovCallStack(lpAddress);
	return s_pfnOrgVirtualFreeEx(hProcess, lpAddress, dwSize, dwFreeType);
}

BOOL WINAPI MyHeapFree(HANDLE hHeap, DWORD dwFlags, LPVOID lpMem)
{
	RemovCallStack(lpMem);
	return s_pfnOrgHeapFree(hHeap, dwFlags, lpMem);
}

HANDLE WINAPI MyGlobalAlloc(UINT uFlags, SIZE_T dwBytes)
{
	HANDLE hHandle = s_pfnOrgGlobalAlloc(uFlags, dwBytes);
...
{
	HANDLE hHandle = s_pfnOrgGlobalReAlloc(hMem, dwBytes, uFlags);
	if (hHandle)
	{
		if (g_HookType == HT_MEMORY)
			CreateCallStack(hHandle, dwBytes);
...
{
	HANDLE hHandle = s_pfnOrgGlobalFree(hMem);
	if (hHandle == NULL)
		RemovCallStack(hMem);
	return hHandle;
...
{
	HLOCAL hHandle = s_pfnOrgLocalAlloc(uFlags, uBytes);
	if (g_HookType == HT_MEMORY)
...
{
	HLOCAL hHandle = s_pfnOrgLocalReAlloc(hMem, uBytes, uFlags);
	if (hHandle)
	{
		if (g_HookType == HT_MEMORY)
			CreateCallStack(hHandle, uBytes);
...
{
	HLOCAL hHandle = s_pfnOrgLocalFree(hMem);
	if (hHandle == NULL)
		RemovCallStack(hMem);
	return hHandle;
}
...
{
	LPVOID lpMem = s_pfnOrgCoTaskMemAlloc(cb);
...
{
	LPVOID lpMem = s_pfnOrgCoTaskMemRealloc(pv, cb);
	if (lpMem)
	{
		CreateCallStack(lpMem, cb);
		RemovCallStack(pv);
...
LPVOID WINAPI MyMapViewOfFile(HANDLE hFileMappingObject
...
{
	LPVOID lMem = s_pfnOrgMapViewOfFile(hFileMappingObject, dwDesiredAccess, dwFileOffsetHigh,
		dwFileOffsetLow, dwNumberOfBytesToMap);
	CreateCallStack(lMem, dwNumberOfBytesToMap);
	return lMem;
}

LPVOID WINAPI MyMapViewOfFileEx(HANDLE hFileMappingObject
	, DWORD dwDesiredAccess
	, DWORD dwFileOffsetHigh
	, DWORD dwFileOffsetLow
	, SIZE_T dwNumberOfBytesToMap
...
{
	LPVOID lMem = s_pfnOrgMapViewOfFileEx(hFileMappingObject, dwDesiredAccess, dwFileOffsetHigh,
...
BOOL WINAPI MyUnmapViewOfFile(LPCVOID lpBaseAddress)
{
	...
	return s_pfnOrgUnmapViewOfFile(lpBaseAddress);
}

INT MyNtMapViewOfSection(HANDLE SectionHandle
...
{
	INT s = s_pfnOrgNtMapViewOfSection(SectionHandle, ProcessHandle, BaseAddress, ZeroBits, CommitSize,
...
{
	INT s = s_pfnOrgNtUnmapViewOfSection(ProcessHandle, BaseAddress);
...
void HookMemAlloc()
{
	...
	/*01*/ REGISTER_HOOK(HeapAlloc, Kernel32);
	/*02*/ REGISTER_HOOK(HeapFree, Kernel32);
	/*03*/ REGISTER_HOOK(HeapReAlloc, Kernel32);
	/*04*/ REGISTER_HOOK(VirtualAlloc, Kernel32);
	/*05*/ REGISTER_HOOK(VirtualFree, Kernel32);
	/*06*/ REGISTER_HOOK(VirtualAllocEx, Kernel32);
	/*07*/ REGISTER_HOOK(VirtualFreeEx, Kernel32);
	/*08*/ REGISTER_HOOK(GlobalAlloc, Kernel32);
	/*09*/ REGISTER_HOOK(GlobalReAlloc, Kernel32);
	/*10*/ REGISTER_HOOK(GlobalFree, Kernel32);
	/*11*/ REGISTER_HOOK(LocalAlloc, Kernel32);
	/*12*/ REGISTER_HOOK(LocalReAlloc, Kernel32);
	/*13*/ REGISTER_HOOK(LocalFree, Kernel32);
	/*14*/ REGISTER_HOOK(MapViewOfFile, Kernel32);
	/*15*/ REGISTER_HOOK(MapViewOfFileEx, Kernel32);
	/*16*/ REGISTER_HOOK(UnmapViewOfFile, Kernel32);
	/*17*/ REGISTER_HOOK(CoTaskMemAlloc, Ole32);
	/*18*/ REGISTER_HOOK(CoTaskMemRealloc, Ole32);
	/*19*/ REGISTER_HOOK(CoTaskMemFree, Ole32);
	/*20*/ ///*REGISTER_HOOK( NtMapViewOfSection, ntdll);
	/*21*/ //REGISTER_HOOK( NtUnmapViewOfSection, ntdll );*/
...
void RestoreHooks()
{
	HookDynamicLoadedFun((UINT)g_arrHookedFunctions.GetCount(), g_arrHookedFunctions.GetData(), true);
}
...
{
	HANDLE hImg = s_pfnOrgLoadImageA(hInst, name, type, cx, cy, fuLoad);
...
{
	HANDLE hImg = s_pfnOrgLoadImageW(hInst, name, type, cx, cy, fuLoad);
	CreateCallStack(hImg, 0);
	return hImg;
}
...
{
	HBITMAP hBmp = s_pfnOrgLoadBitmapA(hInstance, lpBitmapName);
...
{
	HBITMAP hBmp = s_pfnOrgLoadBitmapW(hInstance, lpBitmapName);
	CreateCallStack(hBmp, 0);
	return hBmp;
}
...
{
	HANDLE hBmp = s_pfnOrgLoadImageA(hInst, name, type, cx, cy, fuLoad);
...
{
	HANDLE hBmp = s_pfnOrgLoadImageW(hInst, name, type, cx, cy, fuLoad);
	CreateCallStack(hBmp, type);
	return hBmp;
}
...
{
	HBITMAP hBmp = s_pfnOrgCreateBitmap(nWidth, nHeight, nPlanes, nBitCount, lpBits);
	CreateCallStack(hBmp, 0);
	return hBmp;
}
...
{
	HBITMAP hBmp = s_pfnOrgCreateBitmapIndirect(pbm);
	CreateCallStack(hBmp, 0);
	return hBmp;
}
...
{
	HBITMAP hBmp = s_pfnOrgCreateCompatibleBitmap(hdc, cx, cy);
	CreateCallStack(hBmp, 0);
	return hBmp;
}
...
{
	HBITMAP hBmp = s_pfnOrgCreateDIBitmap(hdc, pbmih, flInit, pjBits, pbmi, iUsage);
	CreateCallStack(hBmp, 0);
	return hBmp;
}
...
{
	HBITMAP hBmp = s_pfnOrgCreateDIBSection(hdc, lpbmi, usage, ppvBits, hSection, offset);
	CreateCallStack(hBmp, 0);
	return hBmp;
}
...
{
	HBITMAP hBmp = s_pfnOrgCreateDiscardableBitmap(hdc, cx, cy);
	CreateCallStack(hBmp, 0);
	return hBmp;
}
...
{
	HANDLE hBmp = s_pfnOrgCopyImage(h, type, cx, cy, flags);
	int nType = 0;
	CreateCallStack(hBmp, type);
	return hBmp;
}
...
{
	BOOL bRet = s_pfnOrgGetIconInfo(hIcon, piconinfo);
	if (bRet)
	{
...
{
	BOOL bRet = s_pfnOrgGetIconInfoExA(hicon, piconinfo);
	if (bRet)
	{
		CreateCallStack(piconinfo->hbmColor, 0);
		if (piconinfo->hbmMask)
...
{
	BOOL bRet = s_pfnOrgGetIconInfoExW(hicon, piconinfo);
	if (bRet)
	{
		CreateCallStack(piconinfo->hbmColor, 0);
		if (piconinfo->hbmMask)
...
{
	BOOL bRet = s_pfnOrgDeleteObject(ho);
	if (bRet)
		RemovCallStack(ho);
	return bRet;
}
...
{
	HICON hIconnew = s_pfnOrgCopyIcon(hIcon);
...
{
	HICON hIcon = s_pfnOrgCreateIcon(hInstance, nWidth, nHeight, cPlanes, cBitsPixel, lpbANDbits, lpbXORbits);
...
{
	HICON hIcon = s_pfnOrgCreateIconFromResource(presbits, dwResSize, fIcon, dwVer);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	HICON hIcon = s_pfnOrgCreateIconFromResourceEx(presbits, dwResSize, fIcon, dwVer, cxDesired, cyDesired, Flags);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	HICON hIcon = s_pfnOrgCreateIconIndirect(piconinfo);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	BOOL bRet = s_pfnOrgDestroyIcon(hIcon);
	if (bRet)
...
{
	HICON hIconnew = s_pfnOrgDuplicateIcon(hInst, hIcon);
	CreateCallStack(hIconnew, IMAGE_ICON);
	return hIconnew;
}
...
{
	HICON hIcon = s_pfnOrgExtractAssociatedIconA(hInst, lpIconPath, lpiIcon);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	HICON hIcon = s_pfnOrgExtractAssociatedIconW(hInst, lpIconPath, lpiIcon);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	HICON hIcon = s_pfnOrgExtractAssociatedIconExA(hInst, lpIconPath, lpiIconIndex, lpiIconId);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	HICON hIcon = s_pfnOrgExtractAssociatedIconExW(hInst, lpIconPath, lpiIconIndex, lpiIconId);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	HICON hIcon = s_pfnOrgExtractIconA(hInst, lpszExeFileName, nIconIndex);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	HICON hIcon = s_pfnOrgExtractIconW(hInst, lpszExeFileName, nIconIndex);
	CreateCallStack(hIcon, IMAGE_ICON);
	return hIcon;
}
...
{
	UINT uRet = s_pfnOrgExtractIconExA(lpszFile, nIconIndex, phiconLarge, phiconSmall, nIcons);
...
{
	UINT uRet = s_pfnOrgExtractIconExW(lpszFile, nIconIndex, phiconLarge, phiconSmall, nIcons);
	if (uRet <= 0 || (-1 == nIconIndex && !phiconLarge && !phiconSmall))
	{
		return uRet;
	}
...
{
	HICON hIcon = s_pfnOrgLoadIconA(hInstance, lpIconName);
	if (hIcon && hInstance)
	{
		CreateCallStack(hIcon, IMAGE_ICON);
	}
...
{
	HICON hIcon = s_pfnOrgLoadIconW(hInstance, lpIconName);
	if (hIcon && hInstance)
	{
		CreateCallStack(hIcon, IMAGE_ICON);
	}
...
{
	UINT uRet = s_pfnOrgPrivateExtractIconsA(szFileName, nIconIndex, cxIcon, cyIcon, phicon, piconid, nIcons, flags);
	if (uRet <= 0 || !phicon)
	{
		return uRet;
	}
...
{
	UINT uRet = s_pfnOrgPrivateExtractIconsW(szFileName, nIconIndex, cxIcon, cyIcon, phicon, piconid, nIcons, flags);
	if (uRet <= 0 || !phicon)
	{
		return uRet;
	}
...
{
	HCURSOR hCur = s_pfnOrgCreateCursor(hInst, xHotSpot, yHotSpot, nWidth, nHeight, pvANDPlane, pvXORPlane);
...
{
	HCURSOR hCur = s_pfnOrgLoadCursorA(hInstance, lpCursorName);
	if (hInstance)
	{
		CreateCallStack(hCur, IMAGE_CURSOR);
	}
...
{
	HCURSOR hCur = s_pfnOrgLoadCursorW(hInstance, lpCursorName);
	if (hInstance)
	{
		CreateCallStack(hCur, IMAGE_CURSOR);
	}
...
{
	HCURSOR hCur = s_pfnOrgLoadCursorFromFileA(lpFileName);
	CreateCallStack(hCur, IMAGE_CURSOR);
	return hCur;
}
...
{
	HCURSOR hCur = s_pfnOrgLoadCursorFromFileW(lpFileName);
	CreateCallStack(hCur, IMAGE_CURSOR);
	return hCur;
}
...
{
	BOOL bRet = s_pfnOrgDestroyCursor(hCursor);
	if (bRet)
	{
...
{
	HBRUSH hBr = s_pfnOrgCreateBrushIndirect(plbrush);
...
{
	HBRUSH hBr = s_pfnOrgCreateSolidBrush(color);
	CreateCallStack(hBr, 0);
	return hBr;
}
...
{
	HBRUSH hBr = s_pfnOrgCreatePatternBrush(hbm);
	CreateCallStack(hBr, 0);
	return hBr;
}
...
{
	HBRUSH hBr = s_pfnOrgCreateDIBPatternBrush(h, iUsage);
	CreateCallStack(hBr, 0);
	return hBr;
}
...
{
	HBRUSH hBr = s_pfnOrgCreateDIBPatternBrushPt(lpPackedDIB, iUsage);
	CreateCallStack(hBr, 0);
	return hBr;
}
...
{
	HBRUSH hBr = s_pfnOrgCreateHatchBrush(iHatch, color);
	CreateCallStack(hBr, 0);
	return hBr;
}
...
{
	HDC hDC = s_pfnOrgCreateCompatibleDC(hdc);
...
{
	HDC hDC = s_pfnOrgCreateDCA(pwszDriver, pwszDevice, pszPort, pdm);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	HDC hDC = s_pfnOrgCreateDCW(pwszDriver, pwszDevice, pszPort, pdm);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	HDC hDC = s_pfnOrgCreateICA(pszDriver, pszDevice, pszPort, pdm);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	HDC hDC = s_pfnOrgCreateICW(pszDriver, pszDevice, pszPort, pdm);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	HDC hDC = s_pfnOrgGetDCEx(hWnd, hrgnClip, flags);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	HDC hDC = s_pfnOrgGetWindowDC(hWnd);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	int nRet = s_pfnOrgReleaseDC(hWnd, hDC);
	if (nRet)
		RemovCallStack(hDC);
	return nRet;
}
...
{
	BOOL bRet = s_pfnOrgDeleteDC(hdc);
	if (bRet)
	{
		RemovCallStack(hdc);
	}
...
// font functions
HFONT WINAPI MyCreateFontA(int cHeight
	, int cWidth
	, int cEscapement
	, int cOrientation
	, int cWeight
...
{
	HFONT hFont = s_pfnOrgCreateFontA(cHeight, cWidth, cEscapement, cOrientation, cWeight, bItalic,
...
HFONT WINAPI MyCreateFontW(int cHeight
	, int cWidth
	, int cEscapement
	, int cOrientation
	, int cWeight
...
{
	HFONT hFont = s_pfnOrgCreateFontW(cHeight, cWidth, cEscapement, cOrientation, cWeight, bItalic,
		bUnderline, bStrikeOut, iCharSet, iOutPrecision, iClipPrecision,
		iQuality, iPitchAndFamily, pszFaceName);
	CreateCallStack(hFont, 0);
	return hFont;
...
{
	HFONT hFont = s_pfnOrgCreateFontIndirectA(lplf);
	CreateCallStack(hFont, 0);
	return hFont;
}
...
{
	HFONT hFont = s_pfnOrgCreateFontIndirectW(lplf);
	CreateCallStack(hFont, 0);
	return hFont;
}
...
{
	HDC hDC = s_pfnOrgCreateMetaFileA(pszFile);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	HDC hDC = s_pfnOrgCreateMetaFileW(pszFile);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	HDC hDC = s_pfnOrgCreateEnhMetaFileA(hdc, lpFilename, lprc, lpDesc);
	CreateCallStack(hDC, 0);
	return hDC;
}
...
{
	HDC hDC = s_pfnOrgCreateEnhMetaFileW(hdc, lpFilename, lprc, lpDesc);
	CreateCallStack(hDC, 0);
	return hDC;
...
{
	HENHMETAFILE hMetaFile = s_pfnOrgGetEnhMetaFileA(lpName);
...
{
	HENHMETAFILE hMetaFile = s_pfnOrgGetEnhMetaFileW(lpName);
	CreateCallStack(hMetaFile, 0);
	return hMetaFile;
...
{
	HMETAFILE hMetaFile = s_pfnOrgGetMetaFileA(lpName);
	CreateCallStack(hMetaFile, 0);
	return hMetaFile;
}
...
{
	HMETAFILE hMetaFile = s_pfnOrgGetMetaFileW(lpName);
	CreateCallStack(hMetaFile, 0);
	return hMetaFile;
...
{
	BOOL bRet = s_pfnOrgDeleteMetaFile(hmf);
	if (bRet)
		RemovCallStack(hmf);
	return bRet;
}
...
{
	BOOL bRet = s_pfnOrgDeleteEnhMetaFile(hmf);
	if (bRet)
	{
		RemovCallStack(hmf);
	}
...
{
	HENHMETAFILE hMetaFile = s_pfnOrgCopyEnhMetaFileA(hEnh, lpFileName);
	CreateCallStack(hMetaFile, 0);
	return hMetaFile;
}
...
{
	HENHMETAFILE hMetaFile = s_pfnOrgCopyEnhMetaFileW(hEnh, lpFileName);
	CreateCallStack(hMetaFile, 0);
	return hMetaFile;
}
...
{
	HENHMETAFILE hMetaFile = s_pfnOrgCloseEnhMetaFile(hdc);
	if (hMetaFile)
	{
		RemovCallStack(hdc);
		CreateCallStack(hMetaFile, 0);
...
{
	HMETAFILE hMetaFile = s_pfnOrgCloseMetaFile(hdc);
	if (hMetaFile)
	{
		RemovCallStack(hdc);
		CreateCallStack(hMetaFile, 0);
...
{
	HPEN hGDIObj = s_pfnOrgCreatePen(iStyle, cWidth, color);
...
HPEN WINAPI MyCreatePenIndirect(CONST LOGPEN* plpen)
{
	...
	HPEN hGDIObj = s_pfnOrgCreatePenIndirect(plpen);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HPEN hGDIObj = s_pfnOrgExtCreatePen(iPenStyle, cWidth, plbrush, cStyle, pstyle);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgPathToRegion(hdc);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgCreateEllipticRgn(x1, y1, x2, y2);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgCreateEllipticRgnIndirect(lprect);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgCreatePolygonRgn(pptl, cPoint, iMode);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgCreatePolyPolygonRgn(pptl, pc, cPoly, iMode);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgCreateRectRgn(x1, y1, x2, y2);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgCreateRectRgnIndirect(lprect);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgCreateRoundRectRgn(x1, y1, x2, y2, w, h);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HRGN hGDIObj = s_pfnOrgExtCreateRegion(lpx, nCount, lpData);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HPALETTE hGDIObj = s_pfnOrgCreateHalftonePalette(hdc);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}
...
{
	HPALETTE hGDIObj = s_pfnOrgCreatePalette(plpal);
	CreateCallStack(hGDIObj, 0);
	return hGDIObj;
}


const int GDI_FUNC_NO = 88;
void HookGDIAlloc()
{
	...
	// Bitmap functions
	/*01*/ REGISTER_HOOK(LoadBitmapA, User32);
	/*02*/ REGISTER_HOOK(LoadBitmapW, User32);
	/*03*/ REGISTER_HOOK(LoadImageA, User32);
	/*04*/ REGISTER_HOOK(LoadImageW, User32);
	/*05*/ REGISTER_HOOK(CreateBitmap, Gdi32);
	/*06*/ REGISTER_HOOK(CreateBitmapIndirect, Gdi32);
	/*07*/ REGISTER_HOOK(CreateCompatibleBitmap, Gdi32);
	/*08*/ REGISTER_HOOK(CreateDIBitmap, Gdi32);
	/*09*/ REGISTER_HOOK(CreateDIBSection, Gdi32);
	/*10*/ REGISTER_HOOK(CreateDiscardableBitmap, Gdi32);

	/*11*/ REGISTER_HOOK(CopyImage, User32);
	/*12*/ REGISTER_HOOK(GetIconInfo, User32);
	/*13*/ REGISTER_HOOK(GetIconInfoExA, User32);
	/*14*/ REGISTER_HOOK(GetIconInfoExA, User32);
	/*15*/ REGISTER_HOOK(DeleteObject, Gdi32);

	//ICONS
	/*16*/ REGISTER_HOOK(CopyIcon, User32);
	/*17*/ REGISTER_HOOK(CreateIcon, User32);
	/*18*/ REGISTER_HOOK(CreateIconFromResource, User32);
	/*19*/ REGISTER_HOOK(CreateIconFromResourceEx, User32);
	/*20*/ REGISTER_HOOK(CreateIconIndirect, User32);

	/*21*/ REGISTER_HOOK(DestroyIcon, User32);
	/*22*/ REGISTER_HOOK(DuplicateIcon, Shell32);
	/*23*/ REGISTER_HOOK(ExtractAssociatedIconA, Shell32);
	/*24*/ REGISTER_HOOK(ExtractAssociatedIconW, Shell32);
	/*25*/ REGISTER_HOOK(ExtractAssociatedIconExA, Shell32);
	/*26*/ REGISTER_HOOK(ExtractAssociatedIconExW, Shell32);
	/*27*/ REGISTER_HOOK(ExtractIconA, Shell32);
	/*28*/ REGISTER_HOOK(ExtractIconW, Shell32);
	/*29*/ REGISTER_HOOK(ExtractIconExA, Shell32);
	/*30*/ REGISTER_HOOK(ExtractIconExW, Shell32);

	/*31*/ REGISTER_HOOK(LoadIconA, User32);
	/*32*/ REGISTER_HOOK(LoadIconW, User32);
	/*33*/ REGISTER_HOOK(PrivateExtractIconsA, User32);
	/*34*/ REGISTER_HOOK(PrivateExtractIconsW, User32);

	// Cursor
	/*35*/ REGISTER_HOOK(CreateCursor, User32);
	/*36*/ REGISTER_HOOK(LoadCursorA, User32);
	/*37*/ REGISTER_HOOK(LoadCursorW, User32);
	/*38*/ REGISTER_HOOK(LoadCursorFromFileA, User32);
	/*39*/ REGISTER_HOOK(LoadCursorFromFileW, User32);
	/*40*/ REGISTER_HOOK(DestroyCursor, User32);


	// brush
	/*41*/ REGISTER_HOOK(CreateBrushIndirect, Gdi32);
	/*42*/ REGISTER_HOOK(CreateSolidBrush, Gdi32);
	/*43*/ REGISTER_HOOK(CreatePatternBrush, Gdi32);
	/*44*/ REGISTER_HOOK(CreateDIBPatternBrush, Gdi32);
	/*45*/ REGISTER_HOOK(CreateDIBPatternBrushPt, Gdi32);
	/*46*/ REGISTER_HOOK(CreateHatchBrush, Gdi32);

	// DC
	/*47*/ REGISTER_HOOK(CreateCompatibleDC, Gdi32);
	/*48*/ REGISTER_HOOK(CreateDCA, Gdi32);
	/*49*/ REGISTER_HOOK(CreateDCW, Gdi32);
	/*50*/ REGISTER_HOOK(CreateICA, Gdi32);

	/*51*/ REGISTER_HOOK(CreateICW, Gdi32);
	/*52*/ REGISTER_HOOK(GetDC, User32);
	/*53*/ REGISTER_HOOK(GetDCEx, User32);
	/*54*/ REGISTER_HOOK(GetWindowDC, User32);
	/*55*/ REGISTER_HOOK(ReleaseDC, User32);
	/*56*/ REGISTER_HOOK(DeleteDC, Gdi32);


	// FONT
	/*57*/ REGISTER_HOOK(CreateFontA, Gdi32);
	/*58*/ REGISTER_HOOK(CreateFontW, Gdi32);
	/*59*/ REGISTER_HOOK(CreateFontIndirectA, Gdi32);
	/*60*/ REGISTER_HOOK(CreateFontIndirectW, Gdi32);

	// Metafile
	/*61*/ REGISTER_HOOK(CreateMetaFileA, Gdi32);
	/*62*/ REGISTER_HOOK(CreateMetaFileW, Gdi32);
	/*63*/ REGISTER_HOOK(CreateEnhMetaFileA, Gdi32);
	/*64*/ REGISTER_HOOK(CreateEnhMetaFileW, Gdi32);
	/*65*/ REGISTER_HOOK(GetEnhMetaFileA, Gdi32);
	/*66*/ REGISTER_HOOK(GetEnhMetaFileW, Gdi32);
	/*67*/ REGISTER_HOOK(GetMetaFileA, Gdi32);
	/*68*/ REGISTER_HOOK(GetMetaFileW, Gdi32);
	/*69*/ REGISTER_HOOK(DeleteMetaFile, Gdi32);
	/*70*/ REGISTER_HOOK(DeleteEnhMetaFile, Gdi32);

	/*71*/ REGISTER_HOOK(CopyEnhMetaFileA, Gdi32);
	/*72*/ REGISTER_HOOK(CopyEnhMetaFileW, Gdi32);
	/*73*/ REGISTER_HOOK(CloseEnhMetaFile, Gdi32);
	/*74*/ REGISTER_HOOK(CloseMetaFile, Gdi32);

	//Pen
	/*75*/ REGISTER_HOOK(CreatePen, Gdi32);
	/*76*/ REGISTER_HOOK(CreatePenIndirect, Gdi32);
	/*77*/ REGISTER_HOOK(ExtCreatePen, Gdi32);

	//region
	/*78*/ REGISTER_HOOK(PathToRegion, Gdi32);
	/*79*/ REGISTER_HOOK(CreateEllipticRgn, Gdi32);
	/*80*/ REGISTER_HOOK(CreateEllipticRgnIndirect, Gdi32);

	/*81*/ REGISTER_HOOK(CreatePolygonRgn, Gdi32);
	/*82*/ REGISTER_HOOK(CreatePolyPolygonRgn, Gdi32);
	/*83*/ REGISTER_HOOK(CreateRectRgn, Gdi32);
	/*84*/ REGISTER_HOOK(CreateRectRgnIndirect, Gdi32);
	/*85*/ REGISTER_HOOK(CreateRoundRectRgn, Gdi32);
	/*86*/ REGISTER_HOOK(ExtCreateRegion, Gdi32);

	//palette
	/*87*/ REGISTER_HOOK(CreateHalftonePalette, Gdi32);
	/*88*/ REGISTER_HOOK(CreatePalette, Gdi32);
...
{
	HANDLE hHandle = s_pfnOrgCreateEventA(lpEventAttributes, bManualReset, bInitialState, lpName);
...
{
	HANDLE hHandle = s_pfnOrgCreateEventW(lpEventAttributes, bManualReset, bInitialState, lpName);
	CreateCallStack(hHandle, TYPE_EVENT_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateEventExA(lpEventAttributes, lpName, dwFlags, dwDesiredAccess);
	CreateCallStack(hHandle, TYPE_EVENT_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateEventExW(lpEventAttributes, lpName, dwFlags, dwDesiredAccess);
	CreateCallStack(hHandle, TYPE_EVENT_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenEventA(dwDesiredAccess, bInheritHandle, lpName);
	CreateCallStack(hHandle, TYPE_EVENT_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenEventW(dwDesiredAccess, bInheritHandle, lpName);
	CreateCallStack(hHandle, TYPE_EVENT_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateMutexA(lpMutexAttributes, bInitialOwner, lpName);
...
{
	HANDLE hHandle = s_pfnOrgCreateMutexW(lpMutexAttributes, bInitialOwner, lpName);
	CreateCallStack(hHandle, TYPE_MUTEX_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateMutexExA(lpEventAttributes, lpName, dwFlags, dwDesiredAccess);
	CreateCallStack(hHandle, TYPE_MUTEX_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateMutexExW(lpEventAttributes, lpName, dwFlags, dwDesiredAccess);
	CreateCallStack(hHandle, TYPE_MUTEX_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenMutexA(dwDesiredAccess, bInheritHandle, lpName);
	CreateCallStack(hHandle, TYPE_MUTEX_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenMutexW(dwDesiredAccess, bInheritHandle, lpName);
	CreateCallStack(hHandle, TYPE_MUTEX_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateSemaphoreA(lpSemaphoreAttributes, lInitialCount, lMaximumCount, lpName);
...
{
	HANDLE hHandle = s_pfnOrgCreateSemaphoreW(lpSemaphoreAttributes, lInitialCount, lMaximumCount, lpName);
	CreateCallStack(hHandle, TYPE_SEMAPHOR_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateSemaphoreExA(lpSemaphoreAttributes, lInitialCount, lMaximumCount, lpName, dwFlags, dwDesiredAccess);
	CreateCallStack(hHandle, TYPE_SEMAPHOR_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateSemaphoreExW(lpSemaphoreAttributes, lInitialCount, lMaximumCount, lpName, dwFlags, dwDesiredAccess);
	CreateCallStack(hHandle, TYPE_SEMAPHOR_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenSemaphoreA(dwDesiredAccess, bInheritHandle, lpName);
	CreateCallStack(hHandle, TYPE_SEMAPHOR_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenSemaphoreW(dwDesiredAccess, bInheritHandle, lpName);
	CreateCallStack(hHandle, TYPE_SEMAPHOR_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateWaitableTimerA(lpTimerAttributes, bManualReset, lpTimerName);
...
{
	HANDLE hHandle = s_pfnOrgCreateWaitableTimerW(lpTimerAttributes, bManualReset, lpTimerName);
	CreateCallStack(hHandle, TYPE_WAIT_TIMER_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateWaitableTimerExA(lpTimerAttributes, lpTimerName, dwFlags, dwDesiredAccess);
	CreateCallStack(hHandle, TYPE_WAIT_TIMER_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateWaitableTimerExW(lpTimerAttributes, lpTimerName, dwFlags, dwDesiredAccess);
	CreateCallStack(hHandle, TYPE_WAIT_TIMER_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenWaitableTimerA(dwDesiredAccess, bInheritHandle, lpTimerName);
	CreateCallStack(hHandle, TYPE_WAIT_TIMER_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenWaitableTimerW(dwDesiredAccess, bInheritHandle, lpTimerName);
	CreateCallStack(hHandle, TYPE_WAIT_TIMER_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateFileA(lpFileName, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile);
...
{
	HANDLE hHandle = s_pfnOrgCreateFileW(lpFileName, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateFileTransactedA(lpFileName, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile, hTransaction, pusMiniVersion, lpExtendedParameter);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateFileTransactedW(lpFileName, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile, hTransaction, pusMiniVersion, lpExtendedParameter);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstFileA(lpFileName, lpFindFileData);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstFileW(lpFileName, lpFindFileData);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstFileExA(lpFileName, fInfoLevelId, lpFindFileData, fSearchOp, lpSearchFilter, dwAdditionalFlags);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstFileExW(lpFileName, fInfoLevelId, lpFindFileData, fSearchOp, lpSearchFilter, dwAdditionalFlags);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstFileNameTransactedW(lpFileName, dwFlags, StringLength, LinkName, hTransaction);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstFileNameW(lpFileName, dwFlags, StringLength, LinkName);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstFileTransactedA(lpFileName, fInfoLevelId, lpFindFileData, fSearchOp, lpSearchFilter, dwAdditionalFlags, hTransaction);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstFileTransactedW(lpFileName, fInfoLevelId, lpFindFileData, fSearchOp, lpSearchFilter, dwAdditionalFlags, hTransaction);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstStreamTransactedW(lpFileName, InfoLevel, lpFindStreamData, dwFlags, hTransaction);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstStreamW(lpFileName, InfoLevel, lpFindStreamData, dwFlags);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	BOOL bRet = s_pfnOrgFindClose(hFindFile);
	if (bRet)
...
{
	HANDLE hHandle = s_pfnOrgOpenFileById(hFile, lpFileID, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwFlags);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgReOpenFile(hOriginalFile, dwDesiredAccess, dwShareMode, dwFlags);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateIoCompletionPort(FileHandle, ExistingCompletionPort, CompletionKey, NumberOfConcurrentThreads);
	CreateCallStack(hHandle, TYPE_FILE_HANDLE);
	return hHandle;
}
...
{
	BOOL   bret = s_pfnOrgCreateRestrictedToken(ExistingTokenHandle, Flags, DisableSidCount, SidsToDisable, DeletePrivilegeCount, PrivilegesToDelete, RestrictedSidCount, SidsToRestrict, NewTokenHandle);
	if (bret)
...
{
	BOOL   bret = s_pfnOrgDuplicateToken(ExistingTokenHandle, ImpersonationLevel, DuplicateTokenHandle);
	if (bret)
...
{
	BOOL   bret = s_pfnOrgDuplicateTokenEx(hExistingToken, dwDesiredAccess, lpTokenAttributes, ImpersonationLevel, TokenType, phNewToken);
	if (bret)
...
{
	BOOL   bret = s_pfnOrgOpenProcessToken(ProcessHandle, DesiredAccess, TokenHandle);
	if (bret)
...
{
	BOOL   bret = s_pfnOrgOpenThreadToken(ThreadHandle, DesiredAccess, OpenAsSelf, TokenHandle);
	if (bret)
		CreateCallStack(*TokenHandle, TYPE_TOKEN_HANDLE);
	return bret;
}
...
{
	HANDLE hHandle = s_pfnOrgFindFirstChangeNotificationA(lpPathName, bWatchSubtree, dwNotifyFilter);
...
{
	HANDLE hHandle = s_pfnOrgFindFirstChangeNotificationW(lpPathName, bWatchSubtree, dwNotifyFilter);
	CreateCallStack(hHandle, TYPE_CHANGE_NOFICATION_HANDLE);
	return hHandle;
}
...
{
	BOOL   bRet = s_pfnOrgFindCloseChangeNotification(hChangeHandle);
	if (bRet)
...
{
	HANDLE hHandle = s_pfnOrgCreateMemoryResourceNotification(NotificationType);
...
{
	HANDLE hHandle = s_pfnOrgCreateFileMappingA(hFile, lpFileMappingAttributes, flProtect, dwMaximumSizeHigh, dwMaximumSizeLow, lpName);
	CreateCallStack(hHandle, TYPE_MEMEORY_MAPPED_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateFileMappingW(hFile, lpFileMappingAttributes, flProtect, dwMaximumSizeHigh, dwMaximumSizeLow, lpName);
	CreateCallStack(hHandle, TYPE_MEMEORY_MAPPED_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateFileMappingNumaA(hFile, lpFileMappingAttributes, flProtect, dwMaximumSizeHigh, dwMaximumSizeLow, lpName, nndPreferred);
	CreateCallStack(hHandle, TYPE_MEMEORY_MAPPED_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateFileMappingNumaW(hFile, lpFileMappingAttributes, flProtect, dwMaximumSizeHigh, dwMaximumSizeLow, lpName, nndPreferred);
	CreateCallStack(hHandle, TYPE_MEMEORY_MAPPED_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenFileMappingA(dwDesiredAccess, bInheritHandle, lpName);
	CreateCallStack(hHandle, TYPE_MEMEORY_MAPPED_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenFileMappingW(dwDesiredAccess, bInheritHandle, lpName);
	CreateCallStack(hHandle, TYPE_MEMEORY_MAPPED_FILE_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgHeapCreate(flOptions, dwInitialSize, dwMaximumSize);
	CreateCallStack(hHandle, TYPE_MEMORY_HANDLE);
	return hHandle;
}
...
{
	BOOL   bRet = s_pfnOrgHeapDestroy(hHeap);
	if (bRet)
...
{
	BOOL   bret = s_pfnOrgCreateProcessA(lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation);
...
{
	BOOL   bret = s_pfnOrgCreateProcessW(lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation);
	if (bret && lpProcessInformation)
	{
		CreateCallStack(lpProcessInformation->hProcess, TYPE_PROCESS_HANDLE);
		CopyStack(lpProcessInformation->hProcess, lpProcessInformation->hThread, TYPE_THREAD_HANDLE);
...
{
	BOOL   bret = s_pfnOrgCreateProcessAsUserA(hToken, lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation);
	if (bret && lpProcessInformation)
	{
		CreateCallStack(lpProcessInformation->hProcess, TYPE_PROCESS_HANDLE);
		CopyStack(lpProcessInformation->hProcess, lpProcessInformation->hThread, TYPE_THREAD_HANDLE);
...
{
	BOOL   bret = s_pfnOrgCreateProcessAsUserW(hToken, lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes, bInheritHandles, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation);
	if (bret && lpProcessInformation)
	{
		CreateCallStack(lpProcessInformation->hProcess, TYPE_PROCESS_HANDLE);
		CopyStack(lpProcessInformation->hProcess, lpProcessInformation->hThread, TYPE_THREAD_HANDLE);
...
{
	BOOL   bret = s_pfnOrgCreateProcessWithLogonW(lpUsername, lpDomain, lpPassword, dwLogonFlags, lpApplicationName, lpCommandLine, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation);
	if (bret && lpProcessInformation)
	{
		CreateCallStack(lpProcessInformation->hProcess, TYPE_PROCESS_HANDLE);
		CopyStack(lpProcessInformation->hProcess, lpProcessInformation->hThread, TYPE_THREAD_HANDLE);
...
{
	BOOL   bret = s_pfnOrgCreateProcessWithTokenW(hToken, dwLogonFlags, lpApplicationName, lpCommandLine, dwCreationFlags, lpEnvironment, lpCurrentDirectory, lpStartupInfo, lpProcessInformation);
	if (bret && lpProcessInformation)
	{
		CreateCallStack(lpProcessInformation->hProcess, TYPE_PROCESS_HANDLE);
		CopyStack(lpProcessInformation->hProcess, lpProcessInformation->hThread, TYPE_THREAD_HANDLE);
...
{
	HANDLE hHandle = s_pfnOrgOpenProcess(dwDesiredAccess, bInheritHandle, dwProcessId);
...
{
	HANDLE hHandle = s_pfnOrgCreateThread(lpThreadAttributes, dwStackSize, lpStartAddress, lpParameter, dwCreationFlags, lpThreadId);
...
{
	HANDLE hHandle = s_pfnOrgCreateRemoteThread(hProcess, lpThreadAttributes, dwStackSize, lpStartAddress, lpParameter, dwCreationFlags, lpThreadId);
	CreateCallStack(hHandle, TYPE_THREAD_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgOpenThread(dwDesiredAccess, bInheritHandle, dwThreadId);
	CreateCallStack(hHandle, TYPE_THREAD_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateJobObjectA(lpJobAttributes, lpName);
...
{
	HANDLE hHandle = s_pfnOrgCreateJobObjectW(lpJobAttributes, lpName);
	CreateCallStack(hHandle, TYPE_JOB_HANDLE);
	return hHandle;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateMailslotA(lpName, nMaxMessageSize, lReadTimeout, lpSecurityAttributes);
...
{
	HANDLE hHandle = s_pfnOrgCreateMailslotW(lpName, nMaxMessageSize, lReadTimeout, lpSecurityAttributes);
	CreateCallStack(hHandle, TYPE_MAIL_SLOT_HANDLE);
	return hHandle;
}
...
{
	BOOL   bret = s_pfnOrgCreatePipe(hReadPipe, hWritePipe, lpPipeAttributes, nSize);
	if (bret)
	{
...
{
	HANDLE hHandle = s_pfnOrgCreateNamedPipeA(lpName, dwOpenMode, dwPipeMode, nMaxInstances, nOutBufferSize, nInBufferSize, nDefaultTimeOut, lpSecurityAttributes);
...
{
	HANDLE hHandle = s_pfnOrgCreateNamedPipeW(lpName, dwOpenMode, dwPipeMode, nMaxInstances, nOutBufferSize, nInBufferSize, nDefaultTimeOut, lpSecurityAttributes);
	CreateCallStack(hHandle, TYPE_PIPE_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegCreateKeyExA(hKey, lpSubKey, Reserved, lpClass, dwOptions, samDesired, lpSecurityAttributes, phkResult, lpdwDisposition);
	if (phkResult)
...
{
	LSTATUS hHandle = s_pfnOrgRegCreateKeyExW(hKey, lpSubKey, Reserved, lpClass, dwOptions, samDesired, lpSecurityAttributes, phkResult, lpdwDisposition);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegCreateKeyTransactedA(hKey, lpSubKey, Reserved, lpClass, dwOptions, samDesired, lpSecurityAttributes, phkResult, lpdwDisposition, hTransaction, pExtendedParemeter);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegCreateKeyTransactedW(hKey, lpSubKey, Reserved, lpClass, dwOptions, samDesired, lpSecurityAttributes, phkResult, lpdwDisposition, hTransaction, pExtendedParemeter);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegOpenCurrentUser(samDesired, phkResult);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegOpenKeyA(hKey, lpSubKey, phkResult);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegOpenKeyW(hKey, lpSubKey, phkResult);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegOpenKeyExA(hKey, lpSubKey, ulOptions, samDesired, phkResult);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegOpenKeyExW(hKey, lpSubKey, ulOptions, samDesired, phkResult);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegOpenKeyTransactedA(hKey, lpSubKey, ulOptions, samDesired, phkResult, hTransaction, pExtendedParemeter);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegOpenKeyTransactedW(hKey, lpSubKey, ulOptions, samDesired, phkResult, hTransaction, pExtendedParemeter);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegOpenUserClassesRoot(hToken, dwOptions, samDesired, phkResult);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegCreateKeyA(hKey, lpSubKey, phkResult);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS hHandle = s_pfnOrgRegCreateKeyW(hKey, lpSubKey, phkResult);
	if (phkResult)
		CreateCallStack(*phkResult, TYPE_REGISTRY_HANDLE);
	return hHandle;
}
...
{
	LSTATUS bRet = s_pfnOrgRegCloseKey(hKey);
	if (bRet)
		RemovCallStack(hKey);
	return bRet;
}
...
{
	HANDLE hHandle = s_pfnOrgCreateTimerQueue();
	if (hHandle)
...
{
	BOOL bRet = s_pfnOrgCreateTimerQueueTimer(phNewTimer, TimerQueue, Callback, Parameter, DueTime, Period, Flags);
...
{
	BOOL bRet = s_pfnOrgDeleteTimerQueueTimer(TimerQueue, Timer, CompletionEvent);
	if (bRet)
...
{
	BOOL bRet = s_pfnOrgDeleteTimerQueueEx(TimerQueue, CompletionEvent);
	if (bRet)
...
{
	BOOL bRet = s_pfnOrgDeleteTimerQueue(TimerQueue);
	if (bRet)
		RemovCallStack(TimerQueue);
	return bRet;
}
...
{
	s_pfnOrgInitializeCriticalSection(lpCriticalSection);
...
{
	BOOL bRet = s_pfnOrgInitializeCriticalSectionEx(lpCriticalSection, dwSpinCount, Flags);
	if (bRet)
		CreateCallStack(lpCriticalSection, TYPE_CRITICAL_SECTION_HANDLE);
	return bRet;
}
...
{
	BOOL bRet = s_pfnOrgInitializeCriticalSectionAndSpinCount(lpCriticalSection, dwSpinCount);
	if (bRet)
		CreateCallStack(lpCriticalSection, TYPE_CRITICAL_SECTION_HANDLE);
	return bRet;
}
...
{
	s_pfnOrgDeleteCriticalSection(lpCriticalSection);
...
{
	BOOL   bret = s_pfnOrgDuplicateHandle(hSourceProcessHandle, hSourceHandle, hTargetProcessHandle, lpTargetHandle, dwDesiredAccess, bInheritHandle, dwOptions);
...
{
	BOOL   bRet = s_pfnOrgCloseHandle(hObject);
	if (bRet)
...
void HookHandleAlloc()
{
	...
	/*01*/ REGISTER_HOOK(CreateEventA, Kernel32);
	/*02*/ REGISTER_HOOK(CreateEventW, Kernel32);
	/*03*/ REGISTER_HOOK(CreateEventExA, Kernel32);
	/*04*/ REGISTER_HOOK(CreateEventExW, Kernel32);
	/*05*/ REGISTER_HOOK(OpenEventA, Kernel32);
	/*06*/ REGISTER_HOOK(OpenEventW, Kernel32);
	/*07*/ REGISTER_HOOK(CreateMutexA, Kernel32);
	/*08*/ REGISTER_HOOK(CreateMutexW, Kernel32);
	/*09*/ REGISTER_HOOK(CreateMutexExA, Kernel32);
	/*10*/ REGISTER_HOOK(CreateMutexExW, Kernel32);
	/*11*/ REGISTER_HOOK(OpenMutexA, Kernel32);
	/*12*/ REGISTER_HOOK(OpenMutexW, Kernel32);
	/*13*/ REGISTER_HOOK(CreateSemaphoreA, Kernel32);
	/*14*/ REGISTER_HOOK(CreateSemaphoreW, Kernel32);
	/*15*/ REGISTER_HOOK(CreateSemaphoreExA, Kernel32);
	/*16*/ REGISTER_HOOK(CreateSemaphoreExW, Kernel32);
	/*17*/ REGISTER_HOOK(OpenSemaphoreA, Kernel32);
	/*18*/ REGISTER_HOOK(OpenSemaphoreW, Kernel32);
	/*19*/ REGISTER_HOOK(CreateWaitableTimerA, Kernel32);
	/*20*/ REGISTER_HOOK(CreateWaitableTimerW, Kernel32);
	/*21*/ REGISTER_HOOK(CreateWaitableTimerExA, Kernel32);
	/*22*/ REGISTER_HOOK(CreateWaitableTimerExW, Kernel32);
	/*23*/ REGISTER_HOOK(OpenWaitableTimerA, Kernel32);
	/*24*/ REGISTER_HOOK(OpenWaitableTimerW, Kernel32);
	/*25*/ REGISTER_HOOK(CreateFileA, Kernel32);
	/*26*/ REGISTER_HOOK(CreateFileW, Kernel32);
	/*27*/ REGISTER_HOOK(CreateFileTransactedA, Kernel32);
	/*28*/ REGISTER_HOOK(CreateFileTransactedW, Kernel32);
	/*29*/ REGISTER_HOOK(FindFirstFileA, Kernel32);
	/*30*/ REGISTER_HOOK(FindFirstFileW, Kernel32);
	/*31*/ REGISTER_HOOK(FindFirstFileExA, Kernel32);
	/*32*/ REGISTER_HOOK(FindFirstFileExW, Kernel32);
	/*33*/ REGISTER_HOOK(FindFirstFileExW, Kernel32);
	/*34*/ REGISTER_HOOK(FindFirstFileNameW, Kernel32);
	/*35*/ REGISTER_HOOK(FindFirstFileTransactedA, Kernel32);
	/*36*/ REGISTER_HOOK(FindFirstFileTransactedW, Kernel32);
	/*37*/ REGISTER_HOOK(FindFirstStreamTransactedW, Kernel32);
	/*38*/ REGISTER_HOOK(FindFirstStreamW, Kernel32);
	/*39*/ REGISTER_HOOK(FindClose, Kernel32);
	/*40*/ REGISTER_HOOK(OpenFileById, Kernel32);
	/*41*/ REGISTER_HOOK(ReOpenFile, Kernel32);
	/*42*/ REGISTER_HOOK(CreateIoCompletionPort, Kernel32);
	/*43*/ REGISTER_HOOK(CreateRestrictedToken, Advapi32);
	/*44*/ REGISTER_HOOK(DuplicateToken, Advapi32);
	/*45*/ REGISTER_HOOK(DuplicateTokenEx, Advapi32);
	/*46*/ REGISTER_HOOK(OpenProcessToken, Advapi32);
	/*47*/ REGISTER_HOOK(OpenThreadToken, Advapi32);
	/*48*/ REGISTER_HOOK(FindFirstChangeNotificationA, Kernel32);
	/*49*/ REGISTER_HOOK(FindFirstChangeNotificationW, Kernel32);
	/*50*/ REGISTER_HOOK(FindCloseChangeNotification, Kernel32);
	/*51*/ REGISTER_HOOK(CreateMemoryResourceNotification, Kernel32);
	/*52*/ REGISTER_HOOK(CreateFileMappingA, Kernel32);
	/*53*/ REGISTER_HOOK(CreateFileMappingW, Kernel32);
	/*54*/ REGISTER_HOOK(CreateFileMappingNumaA, Kernel32);
	/*55*/ REGISTER_HOOK(CreateFileMappingNumaW, Kernel32);
	/*56*/ REGISTER_HOOK(OpenFileMappingA, Kernel32);
	/*57*/ REGISTER_HOOK(OpenFileMappingW, Kernel32);
	/*58*/ REGISTER_HOOK(HeapCreate, Kernel32);
	/*59*/ REGISTER_HOOK(HeapDestroy, Kernel32);
	/*60*/ REGISTER_HOOK(GlobalAlloc, Kernel32);
	/*61*/ REGISTER_HOOK(GlobalReAlloc, Kernel32);
	/*62*/ REGISTER_HOOK(GlobalFree, Kernel32);
	/*63*/ REGISTER_HOOK(LocalAlloc, Kernel32);
	/*64*/ REGISTER_HOOK(LocalReAlloc, Kernel32);
	/*65*/ REGISTER_HOOK(LocalFree, Kernel32);
	/*66*/ REGISTER_HOOK(CreateProcessA, Kernel32);
	/*67*/ REGISTER_HOOK(CreateProcessW, Kernel32);
	/*68*/ REGISTER_HOOK(CreateProcessAsUserA, Advapi32);
	/*69*/ REGISTER_HOOK(CreateProcessAsUserW, Advapi32);
	/*70*/ REGISTER_HOOK(CreateProcessWithLogonW, Advapi32);
	/*71*/ REGISTER_HOOK(CreateProcessWithTokenW, Advapi32);
	/*72*/ REGISTER_HOOK(OpenProcess, Kernel32);
	/*73*/ REGISTER_HOOK(CreateThread, Kernel32);
	/*74*/ REGISTER_HOOK(CreateRemoteThread, Kernel32);
	/*75*/ REGISTER_HOOK(OpenThread, Kernel32);
	/*76*/ REGISTER_HOOK(CreateJobObjectA, Kernel32);
	/*77*/ REGISTER_HOOK(CreateJobObjectW, Kernel32);
	/*78*/ REGISTER_HOOK(CreateMailslotA, Kernel32);
	/*79*/ REGISTER_HOOK(CreateMailslotW, Kernel32);
	/*80*/ REGISTER_HOOK(CreatePipe, Kernel32);
	/*81*/ REGISTER_HOOK(CreateNamedPipeA, Kernel32);
	/*82*/ REGISTER_HOOK(CreateNamedPipeW, Kernel32);
	/*83*/ REGISTER_HOOK(RegCreateKeyExA, Advapi32);
	/*84*/ REGISTER_HOOK(RegCreateKeyExW, Advapi32);
	/*85*/ REGISTER_HOOK(RegCreateKeyTransactedA, Advapi32);
	/*86*/ REGISTER_HOOK(RegCreateKeyTransactedW, Advapi32);
	/*87*/ REGISTER_HOOK(RegOpenCurrentUser, Advapi32);
	/*88*/ REGISTER_HOOK(RegOpenKeyA, Advapi32);
	/*89*/ REGISTER_HOOK(RegOpenKeyW, Advapi32);
	/*90*/ REGISTER_HOOK(RegOpenKeyExA, Advapi32);
	/*91*/ REGISTER_HOOK(RegOpenKeyExW, Advapi32);
	/*92*/ REGISTER_HOOK(RegOpenKeyTransactedA, Advapi32);
	/*93*/ REGISTER_HOOK(RegOpenKeyTransactedW, Advapi32);
	/*94*/ REGISTER_HOOK(RegOpenUserClassesRoot, Advapi32);
	/*95*/ REGISTER_HOOK(RegCreateKeyA, Advapi32);
	/*96*/ REGISTER_HOOK(RegCreateKeyW, Advapi32);
	/*97*/ REGISTER_HOOK(RegCloseKey, Advapi32);
	/*98*/ REGISTER_HOOK(DuplicateHandle, Kernel32);
	/*99*/ REGISTER_HOOK(CloseHandle, Kernel32);

	////////////////////////////////v3 additions//////////////////////////////////////////////////
	// Timers
	/*100*/ REGISTER_HOOK(CreateTimerQueue, Kernel32);
	/*101*/ REGISTER_HOOK(CreateTimerQueueTimer, Kernel32);
	/*102*/ REGISTER_HOOK(DeleteTimerQueueTimer, Kernel32);
	/*103*/ REGISTER_HOOK(DeleteTimerQueueEx, Kernel32);
	/*104*/ REGISTER_HOOK(DeleteTimerQueue, Kernel32);

	/*105*/ REGISTER_HOOK(InitializeCriticalSection, Kernel32);
	/*106*/ REGISTER_HOOK(InitializeCriticalSectionEx, Kernel32);
	/*107*/ REGISTER_HOOK(InitializeCriticalSectionAndSpinCount, Kernel32);
	/*108*/ REGISTER_HOOK(DeleteCriticalSection, Kernel32);
		...
		for (int nIdx = 1; nIdx < nCount; nIdx++)
		{
			...
			if (!SymFromAddr(hProcess, dwOffset, &dwDisplacement, pSymbol))
			{
				...
				if (VirtualQuery((void*)dwOffset, &stMemoryInfo, sizeof(stMemoryInfo))
					!= 0)
				{
					hModule = reinterpret_cast<HMODULE>(stMemoryInfo.AllocationBase);
				}
			...
			if (SymGetLineFromAddr64(hProcess, dwOffset, &dwLine, &Line))
			{
				...
				csLine.Format(csFormatString, csFunctionName, _T(" "), Line.FileName, Line.LineNumber);
			}
			else
			{
				csLine = csFunctionName;
			}

			//            CString csLine = (*(stInfo.parCallStack)).GetAt( nIdx);
			csLine += _T("\r\n");
			File.Write(csLine, csLine.GetLength());
		}
