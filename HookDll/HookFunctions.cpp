...
void FindFunction(HANDLE hModule
	...
	while (pOrigThunk->u1.Function)
	{
		...
		if (IMAGE_ORDINAL_FLAG != (pOrigThunk->u1.Ordinal & IMAGE_ORDINAL_FLAG))
		{
			if (paHookArray[i].szFunc)
			{
				...
				for (i = 0; i < uiCount; i++)
				{
					if (paHookArray[i].szFunc && _strcmpi(paHookArray[i].szFunc, (char*)pByName->Name) == 0)
					{
...
PIMAGE_IMPORT_DESCRIPTOR GetNamedImportDescriptor(HMODULE hModule, LPCSTR szImportMod)
{
	...
	while (pImportDesc->Name)
	{
		...
		if (szCurrMod && _stricmp(szCurrMod, szImportMod) == 0)
			break; // Found it.
